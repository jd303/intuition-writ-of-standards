...
function PsionicsTelepathyPage() {
	...
	/**
	 * Filter: Only Telepathy
	 * */
	const filterByTelepathy = (power) => power.talent == PsionicTalents.Telepathy;
	...
	return (
		<React.Fragment>
			...
			<PageTitle colour="scarlet">
				Psionics &gt; Telepathy <ViewModeToggler></ViewModeToggler>
			</PageTitle>
			<ListingWrapper>
				{powers.filter(filterByTelepathy).map((power, index) => (
					<Listing key={index}>
