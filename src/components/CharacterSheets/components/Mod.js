...
export function Mod( { mod, moveName, clickCallback }) {
	...
	return (
		<div className={st.el}>
			{getStaminaIcon(mod)} {getQuickIcon(mod)} <PurchaseablePointGroup count={1} columns={1} purchased={mod.purchased && 1 || 0} clickCallback={clickCallback} purchaseKey={`${moveName}.${mod.name}`} />  <div className={st.name} onClick={toggleDescriptionVisible}>{mod.name} {mod.type == "Passive" && <>Passive</>}</div>
